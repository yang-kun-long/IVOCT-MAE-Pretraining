"""Clean weighted 4-fold segmentation training, v2.
...
weights during training. V2 adds balanced folds, two-stage encoder unfreezing,
Dice + Focal BCE loss, and post-fold threshold sweep.
"""
import argparse
import json
import sys
...
def weighted_dice_focal_bce_loss(pred_logit, target, weights, alpha=0.75, gamma=2.0):
    ...
    per_sample = config.LAMBDA_DICE * dice_loss + config.LAMBDA_BCE * focal_bce
...
def train_one_epoch_weighted(model, loader, optimizer, scaler, device, sample_weights):
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        with autocast(device_type="cuda", enabled=amp_enabled):
            logits = model(images)
            loss = weighted_dice_focal_bce_loss(logits, masks, weights)
...
def train_fold(fold_info, device, patient_stats, sample_weights, tracker=None, max_epochs=None):
    ...
    best_metrics = None
    patience_counter = 0
    fold_vis_dir = config.SEG_VIS_DIR / f"clean_weighted_v2_fold_{fold_idx}"
    ...
    for epoch in range(epochs):
        ...
        if is_best:
            ...
            patience_counter = 0
            ckpt_path = config.SEG_CHECKPOINT_DIR / f"clean_weighted_v2_fold{fold_idx}_best.pth"
            torch.save({
                "epoch": epoch,
...
def main():
    ...
    experiment_id = f"clean_weighted_4fold_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    print("=" * 80)
    print("Clean Weighted 4-Fold Segmentation Training V2")
    print("=" * 80)
